...
public:
    ...
    void push(std::function<void*()>&& task)
    {
        ...
        auto fut = std::async(std::launch::async,[this,task,thread_id]()
        {
            ...
            std::thread end(&staticThreadPool::release, this, thread_id);
            end.detach();
            --num_busy_;
...
private:
    ...
    std::vector<std::unique_ptr<thread_pool::StaticTaskThread<_Static>>> task_threads_;
